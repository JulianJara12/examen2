...
public class ProductoController {
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<Producto> deletedProducto(@PathVariable String id){
        Producto deleteProducto = productoService.deleteProducto(id);
        if( deleteProducto != null){
            return ResponseEntity.ok(deleteProducto);
        }else{
            return ResponseEntity.notFound().build();
        }
    }
